...
// nvcc G_JuliaExtended_Fixed_Brenna.cu -o temp -lglut -lGL
...
{
    // initial press of button
    if(state != GLUT_DOWN)
    {
        return;
    }

    // convert clicked area to fractal coordinates
    float stepSizeX = (XMax - XMin) / (float)WindowWidth;
    float stepSizeY = (YMax - YMin) / (float)WindowHeight;

    float clickedX = XMin + pixelX * stepSizeX;
    float clickedY = YMin + (WindowHeight - pixelY) * stepSizeY;

    // using different buttons
    float zoomFactor;
    if(button == GLUT_LEFT_BUTTON)
    {
        zoomFactor = 0.5; // zoom in
    }
    else if(button == GLUT_RIGHT_BUTTON)
    {
        zoomFactor = 2.0; // zoom out
    }
    else
    {
        return; // do nothing
    ...
    float halfWidth = (XMax - XMin) * 0.5 * zoomFactor;
    float halfHeight = (YMax - YMin) * 0.5 * zoomFactor;

    XMin = clickedX - halfWidth;
    XMax = clickedX + halfWidth;
    YMin = clickedY - halfHeight;
    YMax = clickedY + halfHeight;
