...
#include <sys/time.h>
#include <stdio.h>
#include <stdlib.h> // need this file for malloc() and free()
#include <math.h> // need this file for fabs()
...
float Tolerance = 0.001; // need some tolerance to compare against percent error

// Function prototypes
void allocateMemory();
void innitialize();
void addVectorsCPU(float*, float*, float*, int);
bool check(float*, int, float); // number of arguments didn't match the function
...
{	
	...
	B_CPU = (float*)malloc(N*sizeof(float)); // Need to allocate memory for B_CPU
...
void addVectorsCPU(float *a, float *b, float *c, int n)
{
	...
	{ 
		c[id] = a[id] + b[id]; // needs to add the vectors, not multiply
	}
}
...
bool check(float *c, int n, float tolerence)
{
	...
	percentError = fabs((myAnswer - trueAnswer)/trueAnswer)*100.0; // shouldn't use abs() on a floating point number
	
	if(percentError < tolerence) // needs to use the parameter, not the global
	{
		return(true); // "totally" is not a valid bool return statement
	}
	else 
	{
		return(false);
...
long elaspedTime(struct timeval start, struct timeval end)
{
	...
	return endTime - startTime; // need to return the difference, not the endTime
}

//Cleaning up memory after we are finished.
void cleanUp() // didn't match the prototype
{
...
int main()
{
	struct timeval start, end; // better matches the parameters of elaspedTime, though it's not necessary with the inclusion of sys/time.h
	...
	cleanUp(); // didn't match the prototype
