...
// nvcc D_VectorAddFixedBlocksAndGrid_Fixed_Brenna.cu -o temp
/*
 What to do:
...
/*
 ...
 1. Hardcoded the blocksize = 256 and gridsize = 64
 2. In addVectorsCPU() and addVectorsGPU(), changed the equation for c = a + b
 3. Made a variable step = blockDim * gridDim; so that this code can handle a larger vector and give each thread more than one job
 4. Added #pragma unroll before the for loop
 5. Included math.h for cos(), sin(), sqrt(), but it runs okay without
*/

// Include files
#include <sys/time.h>
#include <stdio.h>
#include <math.h> // included for cos() and sin() and sqrt()
...
void setUpDevices()
{
	BlockSize.x = 256; // Hard-coded to 256
	BlockSize.y = 1;
	BlockSize.z = 1;
	
	GridSize.x = 64; // Hard-coded to 64
	GridSize.y = 1;
	GridSize.z = 1;
}
...
void addVectorsCPU(float *a, float *b, float *c, int n)
{
	...
	{
		c[id] = sqrt(cos(a[id])*cos(a[id]) + a[id]*a[id] + sin(a[id])*sin(a[id]) - 1.0) + sqrt(cos(b[id])*cos(b[id]) + b[id]*b[id] + sin(b[id])*sin(b[id]) - 1.0); // Making more work so the GPU can "win"
	}
}
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	#pragma unroll // to speed up the for loop
	for(; id < n; id += step) // Added so it can handle a bigger vector than we have threads
	{
		if(id < n) // Staying out of my neighbors yard (he has cameras now)
		{
			c[id] = sqrt(cos(a[id])*cos(a[id]) + a[id]*a[id] + sin(a[id])*sin(a[id]) - 1.0) + sqrt(cos(b[id])*cos(b[id]) + b[id]*b[id] + sin(b[id])*sin(b[id]) - 1.0); // Making more work so the GPU can "win"
		}
	}
