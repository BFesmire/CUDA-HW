...
// To compile: nvcc B_VectorAddGPU1Block_Fixed_Brenna.cu -o temp_b
/*
 What to do:
...
#include <sys/time.h>
#include <stdio.h>
#include <math.h> // include to use fabs() function
...
__global__ void addVectorsGPU(float*, float*, float*, int); // Runs fine, but added * to match the function definition
...
void setUpDevices()
{
	BlockSize.x = 1024; // CUDA can't have more than 1024 threads per block
	BlockSize.y = 1;
	BlockSize.z = 1;
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
	...
	{
		c[id] = a[id] + b[id];
		id += blockDim.x; // First pass, thread 0 has id 0, thread 1 has id 1, etc. Second pass, thread 0 has id 1024, thread 2 has id 1025, etc.
	}
}
...
bool check(float *c, int n, float tolerence)
{
	...
	double trueAnswer;
	double percentError;
	double m = (float)n-1.0; // Needed the -1 because we start at 0.
	...
	percentError = fabs((myAnswer - trueAnswer)/trueAnswer)*100.0; //changed to fabs since it's floating point
	
	if(percentError < tolerence) // checking against the parameter passed into the function
	{
		return(true);
	}
	else 
