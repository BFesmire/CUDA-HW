...
// nvcc C_VectorAddGPUManyBlocks_Fixed_Brenna.cu -o temp
/*
 What to do:
...
/*
 ...
 1. In setUpDevices(), I hard coded Blocksize.x to be 256, as instructed.
 2. In setUpDevices(), I changed Gridsize.x to be (N - 1)/BlockSize.x + 1, as this is the equation we found on Thursday.
	If the Blocksize is 100, and we have 102 threads, this will make the Gridsize to be 2.
3. In addVectorsGPU(), I changed int id to be blockIdx.x * blockDim.x + threadIdx.x. This is another equation that we
	found on Thursday. This one tells the threads which block in the grid to go to. It is very important when we have more
	than one block.
 4. In the function prototypes, I added a float parameter to check() to match how it is used in main.
 5. Included <math.h> so I can use fabs() in the check() function
*/

// Include files
#include <sys/time.h>
#include <stdio.h>
#include <math.h> // added so I could use fabs()
...
bool  check(float*, int, float); // there is a third parameter passed later
...
void setUpDevices()
{
	BlockSize.x = 256; // Changed the value from 101 to 256.
	BlockSize.y = 1;
	BlockSize.z = 1;
	
	GridSize.x = (N - 1)/BlockSize.x + 1; // this adapts how many blocks we need based on the size of the blocks and the number of threads
	GridSize.y = 1;
	GridSize.z = 1;
}
...
{
	int id = blockIdx.x * blockDim.x + threadIdx.x; //This tells the thread which block to go to
...
bool check(float *c, int n, float tolerence)
{
	...
	percentError = fabs((myAnswer - trueAnswer)/trueAnswer)*100.0; // changed to fabs()
